...
module Tzispa
  module Experts
    module Image
      ...
      def resize_crop_image(source:, image_size:nil, dest_file:nil)
        image_file = dest_file || source
        if image_size && MiniMagick::Tool::Convert.new do |convert|
            convert.merge! [source, '-resize', image_size, image_file]
          end
        end
        ...
        MiniMagick::Tool::Convert.new do |convert|
          convert.merge! [ image_file, "-crop", "#{crop_info.delete("'")}", "+repage", "-strip", "-interlace", "Plane", "-gaussian-blur", "0.05", "-quality", "80", image_file ]
        end
      end
